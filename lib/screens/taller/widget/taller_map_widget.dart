...
class TallerMapWidget extends StatefulWidget {
  const TallerMapWidget({Key? key, required this.taller}) : super(key: key);
  final Taller taller;

  @override
  State<TallerMapWidget> createState() => _TallerMapWidgetState();
}

class _TallerMapWidgetState extends State<TallerMapWidget> {
  /* Controlado Mapa */
  ...
  void getCurrentLocation() async {
    ...
    location.getLocation().then((LocationData locationData) async {
      ...
      final newMarker = Marker(
        ...
        position: LatLng((widget.taller.ubicacion.x).toDouble(),
            (widget.taller.ubicacion.y).toDouble()),
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < imgList.length; i++) {
      if (imgList[i].id_taller == widget.taller.id) {
                  ...
                  child: ElevatedButton(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text('Taller: ${widget.taller.nombre}'),
                        const Spacer(),
                        Container(
                          padding: const EdgeInsets.only(
  ...
  /* Future _desplegableOrigenDestino(
      ...
      List<Taller> tallerList,
      tallerFormController tallerForm,
      NotificacionService tallerService,
      List<Servicio> servicioList,
      List<Img> imgList) {
    String ServicioNombre = servicioList
        .firstWhere((element) => element.id == widget.taller.id_servicio)
        .servicio;
